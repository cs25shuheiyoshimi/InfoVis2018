function main()
{
    ...
    {
        var id = faces[i];
        var S0 = Normalize(scalars[ id[0] ],min,max);
        var S1 = Normalize(scalars[ id[1] ],min,max);
        var S2 = Normalize(scalars[ id[2] ],min,max);
    ...
    function Normalize(S,min,max){ // S:0.1~0.8 -> S:0~255
    return Math.round((S-min)/(max-min)*255);
    }
